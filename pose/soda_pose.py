...
def get_aruco_t_soda(color_image, depth_image, intrinsic, aruco_t_camera, templates):
    # intrinsic matrix
    fx = intrinsic[0, 0]
    fy = intrinsic[1, 1]
    ppx = intrinsic[0, 2]
    ...
    position_image = cv2.ximgproc.niBlackThreshold(gray_image, maxValue=255, type=cv2.THRESH_BINARY, blockSize=41, k=-0.2)

    # Similarity checking
    min_y, max_y, min_x, max_x = 270, 540, 100, 860
    similarities = [cv2.matchTemplate(position_image[min_y:max_y, min_x:max_x], template, cv2.TM_CCORR_NORMED)
                    for template in templates]
    ...
    for i_similarity, similarity in enumerate(similarities):
        ...
        value = np.max(flat)
        loc = np.argmax(flat)
        peaks.append((value, min_y + templates[i_similarity].shape[0] // 2 + loc // similarity.shape[1],
                      min_x + templates[i_similarity].shape[1] // 2 + loc % similarity.shape[1]))
        if max_peak is None:
            max_peak = peaks[-1]
        elif max_peak[0] < peaks[-1][0]:
            max_peak = peaks[-1]
    ...
    camera_t_aruco = se3.t_to_invt(aruco_t_camera)
    camera_t_soda = np.concatenate((np.array([x, y, z]), camera_t_aruco[3:]), -1)
    aruco_T_soda = se3.t_to_T(aruco_t_camera) @ se3.t_to_T(camera_t_soda)
    aruco_t_soda = se3.T_to_t(aruco_T_soda)
    return aruco_t_soda
